...
function generateRandomPassword(length) {
  ...
  for (let i = 0; i < length; i++) {
    ...
    // str.padEnd(characters.at(index));
    const randomCharacter = characters.charAt(index);
    str += randomCharacter;
  }
  return str;
...
function roundObjectValues(obj) {
  if (!(typeof obj === "object") || obj === null) {
    ...
    const roundedEntries = entries.map(([key, value], index, arr) => {
...
function getFractionalPart(num) {
  if (!(typeof num === "number")) {
    ...
    const FractionalPart = num - Math.trunc(num);
...
function estimateInvestment(principal, interestRate, years) {
  if (
    ...
    const sum = principal * Math.pow(1 + interestRate, years);
...
function isTotalPriceExceedsMaxPrice(products, maxPrice) {
  if (!Array.isArray(products) || !(typeof maxPrice === "number")) {
    console.log("Помилка з аргументами");
    return null;
  } else {
    const totalPrice = products.reduce((totalPrice, value) => {
